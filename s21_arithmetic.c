...
void one_add(s21_decimal *value){
  int tmp = 1;
  for (int i = 0; i <= 96; i++) {
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) { // нет проверки на флаги
    int flag = 0;
    if(s21_last_bit(value_1) + s21_last_bit(value_2) <= 96){
      s21_decimal tmp;
...
//      printf("%u %u %u %u\n", val_og.bits[0],val_og.bits[1],val_og.bits[2], val_og.bits[3]);
      ...
      if (s21_is_greater_or_equal(val_1, val_2)) { 
        ...
        if(s21_is_greater(val_2, val_1)){
          s21_mul(val_1, ten, &val_1);
          s21_mul(*result, ten, result);
          skale++;
      }
      printf("после итерации %u %u %u %u\n", val_1.bits[0],val_1.bits[1],val_1.bits[2], val_1.bits[3]);
      printf("после итерации %u %u %u %u\n", val_2.bits[0],val_2.bits[1],val_2.bits[2], val_2.bits[3]);
      printf("после итерации promej %u %u %u %u\n\n", result->bits[0], result->bits[1],result->bits[2], result->bits[3]);

    }
  }
   s21_setScale(result, s21_getScale(val_1)+skale);
  printf("%u %u %u %u\n", result->bits[0], result->bits[1],result->bits[2], result->bits[3]);
    return flag; 
}
